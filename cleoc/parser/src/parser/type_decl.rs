...
pub fn type_decl<'tokens, 'src: 'tokens, I>()
-> impl Parser<'tokens, I, TypeDecl, extra::Err<ParserError<'tokens, 'src>>> + Clone
where
    I: ValueInput<'tokens, Token = TokenKind<'src>, Span = SimpleSpan>,
{
    ...
    let enum_variant = ident().map(|name| EnumVariant { name });
