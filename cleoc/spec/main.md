...
Marks the function for inlining at call sites. Replaces the `inline` keyword.
...
**TODO (not yet implemented):** The current parser uses the `inline` keyword
(`inline fn ...`). The `inline` keyword will be removed once `@inline` is parsed.
...
**TODO (not yet implemented):** The parser currently accepts `inline fn ...` as a
keyword. This will be removed once `@inline` is implemented.
...
```
trait_decl  = "trait" "{" [fn_signature (";" fn_signature)* [";"]] "}"
...
An unconditional infinite loop. Must be exited with `break`.
...
Plain `break` and `continue` with no label or value.
...
| 8     | unary `*` `&` `-` `!`             | prefix        |
| 9     | `as` (cast)                       | postfix       |
| 10    | `.field` `(args)` (access / call) | left          |

\*Addition, multiplication, and division are implemented with right-recursive parsers,
which gives right-associativity. This is a known issue to revisit.
...
**TODO:** Operator precedence for `+`/`*`/`/` is right-associative due to the
recursive parser structure. It should be left-associative.
...
**TODO:** Whether `if` returns a value (blocks as expressions) is not yet defined
in the parser. Currently the block return type is not propagated.
...
The left-hand side must be an lvalue; this is enforced by the analyzer, not the parser.
...
- `var x = { ...; return value }` (block initializers)
...
| `self`                     | keyword — parsed only as first fn param                              |
...
- **Operator associativity:** `+`, `*`, `/` should be left-associative.
